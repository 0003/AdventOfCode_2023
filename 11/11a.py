...
def simple_galactic_distance(g1,g2,height_expansions,width_expansions) -> int:
    ...
    inflation_height = sum(1 for ix in range(min(i1,i2),max(i1,i2)) if ix in height_expansions)
    inflation_width = sum(1 for ix in range(min(j1,j2),max(j1,j2)) if ix in width_expansions)
